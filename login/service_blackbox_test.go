...
func (s *serviceBlackBoxTest) TestKeycloakAuthorizationRedirect() {
	...
	err = s.loginService.Perform(authorizeCtx, s.oauth, brokerEndpoint, "", "", s.getValidRedirectURLs(), s.configuration.GetHeaderMaxLength())
...
func (s *serviceBlackBoxTest) TestKeycloakAuthorizationWithNoValidRefererFails() {
	...
	err = s.loginService.Perform(authorizeCtx, s.oauth, brokerEndpoint, "", "", config.DefaultValidRedirectURLs, s.configuration.GetHeaderMaxLength())
	assert.Equal(s.T(), 400, rw.Code)
