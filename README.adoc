...
If you are using Go `1.5` ensure the environment variable `GO15VENDOREXPERIMENT`
...
If set, the `GO15VENDOREXPERIMENT` variable tells `go` to look for dependencies
not only in the `GOPATH` (as usual) but also in a `vendor` directory located in
