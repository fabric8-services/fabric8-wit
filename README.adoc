...
Glide expects `GOPATH` environment variable to be set.  It also requires
`PATH` environment updated with `$GOPATH/bin`. e.g.,
...
There is a separate documentation for https://github.com/fabric8-services/fabric8-wit/blob/master/minishift/README.md[building WIT using minishift]
