...
func (s *workItemRevisionRepositoryBlackBoxTest) TestStoreWorkItemRevisions() {
	...
	require.Nil(s.T(), err)
	// when
	workitemRevisions, err := s.workitemRevisionRepository.List(context.Background(), *workItem)
	// then
	require.Nil(s.T(), err)
	...
	assert.Equal(s.T(), workitem.CREATE_WORKITEM_REVISION_TYPE, workitemRevision1.Type)
	...
	assert.Equal(s.T(), workitem.UPDATE_WORKITEM_REVISION_TYPE, workitemRevision2.Type)
	...
	assert.Equal(s.T(), workitem.UPDATE_WORKITEM_REVISION_TYPE, workitemRevision3.Type)
	...
	assert.Equal(s.T(), workitem.DELETE_WORKITEM_REVISION_TYPE, workitemRevision4.Type)
