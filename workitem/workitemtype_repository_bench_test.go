...
func (r *BenchWorkItemTypeRepository) BenchmarkListWorkItemTypes() {
	...
	r.B().ReportAllocs()
	for n := 0; n < r.B().N; n++ {
		if s, err := r.repo.List(context.Background(), fxt.SpaceTemplates[0].ID, nil, nil); err != nil || (err == nil && s == nil) {
			r.B().Fail()
		}
	}
}
...
func (r *BenchWorkItemTypeRepository) BenchmarkListWorkItemTypesTransaction() {
	...
	for n := 0; n < r.B().N; n++ {
		if err := application.Transactional(gormapplication.NewGormDB(r.DB), func(app application.Application) error {
			_, err := r.repo.List(context.Background(), fxt.SpaceTemplates[0].ID, nil, nil)
			return err
		}); err != nil {
			r.B().Fail()
		}
...
// TODO(kwk): I question this benchmark
...
func (r *BenchWorkItemTypeRepository) BenchmarkListFind() {
	...
	for n := 0; n < r.B().N; n++ {
		var rows []workitem.WorkItemType
		db := r.DB.Where("space_template_id = ?", fxt.SpaceTemplates[0].ID)
		if err := db.Find(&rows).Error; err != nil {
			r.B().Fail()
		}
	}
...
func (r *BenchWorkItemTypeRepository) BenchmarkListRawScan() {
	...
	for n := 0; n < r.B().N; n++ {
		var rows []workitem.WorkItemType
		result, err := r.DB.Raw("select * from work_item_types where space_template_id = ?", fxt.SpaceTemplates[0].ID).Rows()
		if err != nil {
			r.B().Fail()
		}
		defer result.Close()
...
func (r *BenchWorkItemTypeRepository) BenchmarkListRawScanName() {
	...
	for n := 0; n < r.B().N; n++ {
		var rows []string
		result, err := r.DB.Raw("select name from work_item_types where space_template_id = ?", fxt.SpaceTemplates[0].ID).Rows()
		if err != nil {
			r.B().Fail()
		}
		defer result.Close()
...
func (r *BenchWorkItemTypeRepository) BenchmarkListRawScanFields() {
	...
	for n := 0; n < r.B().N; n++ {
		...
		result, err := r.DB.Raw("select fields from work_item_types where space_template_id = ?", fxt.SpaceTemplates[0].ID).Rows()
		if err != nil {
			r.B().Fail()
		}
		defer result.Close()
