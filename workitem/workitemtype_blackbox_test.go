...
func TestConstants(t *testing.T) {
	...
	require.Equal(t, "system_remote_item_id", workitem.SystemRemoteItemID)
	require.Equal(t, "system_number", workitem.SystemNumber)
	require.Equal(t, "system_title", workitem.SystemTitle)
	require.Equal(t, "system_description", workitem.SystemDescription)
	require.Equal(t, "system_description_markup", workitem.SystemDescriptionMarkup)
	require.Equal(t, "system_description_rendered", workitem.SystemDescriptionRendered)
	require.Equal(t, "system_state", workitem.SystemState)
	require.Equal(t, "system_assignees", workitem.SystemAssignees)
	require.Equal(t, "system_creator", workitem.SystemCreator)
	require.Equal(t, "system_created_at", workitem.SystemCreatedAt)
	require.Equal(t, "system_updated_at", workitem.SystemUpdatedAt)
	require.Equal(t, "system_order", workitem.SystemOrder)
	require.Equal(t, "system_iteration", workitem.SystemIteration)
	require.Equal(t, "system_area", workitem.SystemArea)
	require.Equal(t, "system_codebase", workitem.SystemCodebase)
	require.Equal(t, "system_labels", workitem.SystemLabels)
	require.Equal(t, "system_boardcolumns", workitem.SystemBoardcolumns)
