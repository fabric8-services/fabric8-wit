...
func (c *CodebaseContent) IsValid() error {
	...
	if c.StackID == "" {
		return errors.NewBadParameterError("system.codebase", StackIdKey+" is mandatory")
	}
	return nil
}
...
func NewCodebaseContent(value map[string]interface{}) (CodebaseContent, error) {
	cb := CodebaseContent{}
	validKeys := []string{RepositoryKey, BranchKey, FileNameKey, LineNumberKey, StackIdKey}
...
type Repository interface {
	...
	ListByStackId(ctx context.Context, spaceID uuid.UUID, stackID string, start *int, limit *int) ([]*Codebase, uint64, error)
...
// ListByStackId all codebases related to spaceID and stackID
func (m *GormCodebaseRepository) ListByStackId(ctx context.Context, spaceID uuid.UUID, stackID string, start *int, limit *int) ([]*Codebase, uint64, error) {
	defer goa.MeasureSince([]string{"goa", "db", "codebase", "query"}, time.Now())

	db := m.db.Model(&Codebase{}).Where("space_id = ? and stack_id = ?", spaceID, stackID)
	orgDB := db
	if start != nil {
		if *start < 0 {
			return nil, 0, errors.NewBadParameterError("start", *start)
