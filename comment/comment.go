...
func (m *GormCommentRepository) Create(ctx context.Context, comment *Comment) error {
	...
	if err := m.db.Create(comment).Error; err != nil {
		log.LoggerRuntimeContext().WithFields(map[string]interface{}{
			"comment": comment,
			"err":     err,
		}).Errorln("Unable to create the comment")
...
func (m *GormCommentRepository) Save(ctx context.Context, comment *Comment) (*Comment, error) {
	...
	if err := tx.Error; err != nil {
		log.LoggerRuntimeContext().WithFields(map[string]interface{}{
			"comment": comment,
			"err":     err,
		}).Errorln("Comment search operation failed!")
	...
	if err := tx.Error; err != nil {
		log.LoggerRuntimeContext().WithFields(map[string]interface{}{
			"comment": comment,
			"err":     err,
		}).Errorln("Unable to save the comment!")
...
func (m *GormCommentRepository) Load(ctx context.Context, id uuid.UUID) (*Comment, error) {
	...
	if tx.Error != nil {
		log.LoggerRuntimeContext().WithFields(map[string]interface{}{
			"id":  id,
			"err": tx.Error,
		}).Errorln("Unable to load the comment")
