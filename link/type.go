...
func (t *WorkItemLinkType) CheckValidForCreation() error {
	if t.Name == "" {
		return models.NewBadParameterError("name", t.Name)
	}
	if t.SourceTypeName == "" {
		return models.NewBadParameterError("source_type_name", t.SourceTypeName)
	}
	if t.TargetTypeName == "" {
		return models.NewBadParameterError("target_type_name", t.TargetTypeName)
	}
	if t.ForwardName == "" {
		return models.NewBadParameterError("forward_name", t.ForwardName)
	}
	if t.ReverseName == "" {
		return models.NewBadParameterError("reverse_name", t.ReverseName)
	}
	if err := CheckValidTopology(t.Topology); err != nil {
		return err
	}
	if t.LinkCategoryID == satoriuuid.Nil {
		return models.NewBadParameterError("link_category_id", t.LinkCategoryID)
	}
	return nil
}
...
func CheckValidTopology(t string) error {
	if t != TopologyNetwork && t != TopologyDirectedNetwork && t != TopologyDependency && t != TopologyTree {
		return models.NewBadParameterError("topolgy", t).Expected(TopologyNetwork + "|" + TopologyDirectedNetwork + "|" + TopologyDependency + "|" + TopologyTree)
	}
	return nil
}
...
func ConvertLinkTypeToModel(in app.WorkItemLinkType, out *WorkItemLinkType) error {
	...
	if in.Data.ID != nil {
		...
		if err != nil {
			...
			return models.NewNotFoundError("work item link type", id.String())
		}
		out.ID = id
	}

	if in.Data.Type != EndpointWorkItemLinkTypes {
		return models.NewBadParameterError("data.type", in.Data.Type).Expected(EndpointWorkItemLinkTypes)
	}
	...
	if attrs != nil {
		...
		if attrs.Name != nil {
			if *attrs.Name == "" {
				return models.NewBadParameterError("data.attributes.name", *attrs.Name)
			}
			out.Name = *attrs.Name
		}
		...
		if attrs.ForwardName != nil {
			if *attrs.ForwardName == "" {
				return models.NewBadParameterError("data.attributes.forward_name", *attrs.ForwardName)
			}
		...
		if attrs.ReverseName != nil {
			if *attrs.ReverseName == "" {
				return models.NewBadParameterError("data.attributes.reverse_name", *attrs.ReverseName)
			}
	...
	if rel != nil && rel.LinkCategory != nil && rel.LinkCategory.Data != nil {
		...
		if d.Type != EndpointWorkItemLinkCategories {
			return models.NewBadParameterError("data.relationships.link_category.data.type", d.Type).Expected(EndpointWorkItemLinkCategories)
		}
		// The the link category MUST NOT be empty
		if d.ID == "" {
			return models.NewBadParameterError("data.relationships.link_category.data.id", d.ID)
		}
		out.LinkCategoryID, err = satoriuuid.FromString(d.ID)
		if err != nil {
			//log.Printf("Error when converting %s to UUID: %s", in.Data.ID, err.Error())
			// treat as not found: clients don't know it must be a UUID
			return models.NotFoundError{entity: "work item link category", ID: d.ID}
		}
	}
	...
	if rel != nil && rel.SourceType != nil && rel.SourceType.Data != nil {
		...
		if d.Type != EndpointWorkItemTypes {
			return models.NewBadParameterError("data.relationships.source_type.data.type", d.Type).Expected(EndpointWorkItemTypes)
		}
		// The the link type MUST NOT be empty
		if d.ID == "" {
			return models.NewBadParameterError("data.relationships.source_type.data.id", d.ID)
		}
	...
	if rel != nil && rel.TargetType != nil && rel.TargetType.Data != nil {
		...
		// If the the link type is not nil, it MUST be "workitemlinktypes"
		if d.Type != EndpointWorkItemTypes {
			return models.NewBadParameterError("data.relationships.target_type.data.type", d.Type).Expected(EndpointWorkItemTypes)
		}
		// The the link type MUST NOT be empty
		if d.ID == "" {
			return models.NewBadParameterError("data.relationships.target_type.data.id", d.ID)
		}
