...
//         fmt.Println(ctx.WorkItemType[0].ID) // WARNING: not yet set
//         return nil
//     }))
// On the other hand, you can safely lookup the space ID when you're in the
// customize-entity-callback for a work item:
//     NewContext(db, WorkItem(1, func(ctx *TestContext, idx int) error{
//         fmt.Println(ctx.Space[0].ID) // safe to access
//         return nil
//     }))
//
