...
func (m *GormIterationRepository) Create(ctx context.Context, u *Iteration) error {
	...
	if err != nil {
		log.LoggerRuntimeContext().WithFields(map[string]interface{}{
			"iteration": u,
			"err":       err,
		}).Errorln("Unable to create the iteration")
...
func (m *GormIterationRepository) List(ctx context.Context, spaceID uuid.UUID) ([]*Iteration, error) {
	...
	if err != nil && err != gorm.ErrRecordNotFound {
		log.LoggerRuntimeContext().WithFields(map[string]interface{}{
			"spaceID": spaceID,
			"err":     err,
		}).Errorln("Unable to list the iterations")
...
func (m *GormIterationRepository) Load(ctx context.Context, id uuid.UUID) (*Iteration, error) {
	...
	if tx.RecordNotFound() {
		log.LoggerRuntimeContext().WithFields(map[string]interface{}{
			"id": id.String(),
		}).Errorln("Iteration cannot be found")
	...
	if tx.Error != nil {
		log.LoggerRuntimeContext().WithFields(map[string]interface{}{
			"id":  id.String(),
			"err": tx.Error,
		}).Errorln("Unable to load the iteration")
...
func (m *GormIterationRepository) Save(ctx context.Context, i Iteration) (*Iteration, error) {
	...
	if err := tx.Error; err != nil {
		log.LoggerRuntimeContext().WithFields(map[string]interface{}{
			"iterationID": i.ID,
			"err":         err,
		}).Errorln("Unknown error happened when searching the iteration")
	...
	if err := tx.Error; err != nil {
		log.LoggerRuntimeContext().WithFields(map[string]interface{}{
			"iteration": i,
			"err":       err,
		}).Errorln("Unable to save the iterations")
		return nil, errors.NewInternalError(err.Error())
	}
	return &i, nil
}
...
func (m *GormIterationRepository) CanStartIteration(ctx context.Context, i *Iteration) (bool, error) {
	...
	if count != 0 {
		log.LoggerRuntimeContext().WithFields(map[string]interface{}{
			"iteration": i,
		}).Errorln("One iteration from given space is already runnin")
