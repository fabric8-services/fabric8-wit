...
func PopulateCommonTypes(ctx context.Context, db *gorm.DB, witr *workitem.GormWorkItemTypeRepository) error {
	...
	if err := createOrUpdatePlannerItemExtension(workitem.SystemUserStory, "User Story", "Desciption for User Story", "fa-mapmarker", ctx, witr, db, space.SystemSpace); err != nil {
		return errs.WithStack(err)
	}
