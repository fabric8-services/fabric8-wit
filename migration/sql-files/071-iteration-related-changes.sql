...
CREATE FUNCTION iteration_set_relationship_timestamp_on_workitem_linking() RETURNS trigger AS $$
    ...
    BEGIN
        UPDATE iterations i SET relationships_changed_at = NEW.updated_at WHERE i.id::text = NEW.Fields->>'system_iteration';
        RETURN NEW;
    END;
$$ LANGUAGE plpgsql;
...
CREATE FUNCTION iteration_set_relationship_timestamp_on_workitem_unlinking() RETURNS trigger AS $$
    -- trigger to fill the `relationships_changed_at` column when an interation is set
    BEGIN
        UPDATE iterations i SET relationships_changed_at = NEW.updated_at WHERE i.id::text = OLD.Fields->>'system_iteration';
        RETURN NEW;
    END;
$$ LANGUAGE plpgsql;
...
CREATE FUNCTION iteration_set_relationship_timestamp_on_workitem_deletion() RETURNS trigger AS $$
    -- trigger to fill the `relationships_changed_at` column when an interation is set
    BEGIN
        UPDATE iterations i SET relationships_changed_at = NEW.deleted_at WHERE i.id::text = OLD.Fields->>'system_iteration';
        RETURN NEW;
    END;
$$ LANGUAGE plpgsql;
