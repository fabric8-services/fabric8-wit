...
CREATE FUNCTION workitem_comment_insert_timestamp() RETURNS trigger AS $$
    -- trigger to fill the `commented_at` column when a comment is added
    BEGIN
...
CREATE FUNCTION workitem_comment_update_timestamp() RETURNS trigger AS $$
    -- trigger to fill the `commented_at` column when a comment is updated 
    BEGIN
...
CREATE FUNCTION workitem_link_insert_timestamp() RETURNS trigger AS $$
    -- trigger to fill the `linked_at` column when a link is added
    BEGIN
...
CREATE FUNCTION workitem_link_update_timestamp() RETURNS trigger AS $$
    -- trigger to fill the `linked_at` column when a link is updated
    BEGIN
        UPDATE work_items wi SET linked_at = NEW.updated_at WHERE wi.id = NEW.source_id;
        UPDATE work_items wi SET linked_at = NEW.updated_at WHERE wi.id = NEW.target_id;
        RETURN NEW;
    END;
$$ LANGUAGE plpgsql;

CREATE FUNCTION workitem_link_softdelete_timestamp() RETURNS trigger AS $$
    -- trigger to fill the `linked_at` column when a link is removed (soft delete, it's a record update)
    BEGIN
...
CREATE TRIGGER workitem_link_update_trigger AFTER UPDATE OF updated_at ON work_item_links 
    FOR EACH ROW
    EXECUTE PROCEDURE workitem_link_update_timestamp();
