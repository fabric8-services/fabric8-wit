...
func (s *workItemTypesSuite) TestList() {
	...
	s.T().Run("ok", func(t *testing.T) {
		...
		page := "0,-1"
		res, witCollection := test.ListWorkitemtypesOK(t, nil, nil, s.typeCtrl, fxt.SpaceTemplates[0].ID, &page, nil, nil)
		// then
	...
	s.T().Run("ok - using expired IfModifiedSince header", func(t *testing.T) {
		...
		page := "0,-1"
		res, witCollection := test.ListWorkitemtypesOK(t, nil, nil, s.typeCtrl, fxt.SpaceTemplates[0].ID, &page, &lastModified, nil)
		// then
		require.NotNil(t, witCollection)
		require.Nil(t, witCollection.Validate())
	...
	s.T().Run("ok - using IfNoneMatch header", func(t *testing.T) {
		...
		etag := "foo"
		page := "0,-1"
		res, witCollection := test.ListWorkitemtypesOK(t, nil, nil, s.typeCtrl, fxt.SpaceTemplates[0].ID, &page, nil, &etag)
		// then
		require.NotNil(t, witCollection)
		require.Nil(t, witCollection.Validate())
	...
	s.T().Run("not modified - using IfModifiedSince header", func(t *testing.T) {
		...
		page := "0,-1"
		test.ListWorkitemtypesNotModified(t, nil, nil, s.typeCtrl, fxt.SpaceTemplates[0].ID, &page, &lastModified, nil)
	})
	...
	s.T().Run("not modified - using IfNoneMatch header", func(t *testing.T) {
		...
		// Paging in the format <start>,<limit>"
		page := "0,-1"
		_, witCollection := test.ListWorkitemtypesOK(t, nil, nil, s.typeCtrl, fxt.SpaceTemplates[0].ID, &page, nil, nil)
		require.NotNil(t, witCollection)
		// when/then
