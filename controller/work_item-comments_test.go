...
func (rest *TestCommentREST) TestSuccessCreateSingleCommentWithMarkup() {
	...
	_, c := test.CreateWorkItemCommentsOK(rest.T(), svc.Context, svc, ctrl, wi.Relationships.SpaceID.String(), wi.ID, p)
	// then
...
func (rest *TestCommentREST) TestListCommentsByParentWorkItem() {
	...
	offset := "0"
	limit := 3
	_, cs := test.ListWorkItemCommentsOK(rest.T(), svc.Context, svc, ctrl, wi.Relationships.SpaceID.String(), wi.ID, &limit, &offset)
	// then
	...
	// when
	_, cs2 := test.ListWorkItemCommentsOK(rest.T(), svc.Context, svc, ctrl, wi2.Relationships.SpaceID.String(), wi2.ID, &limit, &offset)
	// then
...
func (rest *TestCommentREST) TestCreateSingleNoAuthorized() {
	...
	p := rest.newCreateWorkItemCommentsPayload("Test", nil)
	svc, ctrl := rest.UnSecuredController()
	test.CreateWorkItemCommentsUnauthorized(rest.T(), svc.Context, svc, ctrl, wi.Relationships.SpaceID.String(), wi.ID, p)
}
