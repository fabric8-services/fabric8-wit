...
func (s *TestPagingSuite) TestPagingLinks() {
	pagingTest := createPagingTest(s.T(), s.svc.Context, s.controller, s.repo, space.SystemSpace, 13)
	...
	pagingTest = createPagingTest(s.T(), s.svc.Context, s.controller, s.repo, space.SystemSpace, 0)
...
func (s *TestPagingSuite) TestPagingErrors() {
	...
	var offset string = "-1"
	var limit int = 2
	_, result := test.ListWorkitemOK(s.T(), context.Background(), nil, s.controller, space.SystemSpace, nil, nil, nil, nil, nil, nil, nil, nil, &limit, &offset, nil, nil)
...
func (s *TestPagingSuite) TestPagingDefaultAndMaxSize() {
	...
	s.repo.ListReturns(makeWorkItems(10), uint64(100), nil)
	// when
	_, result := test.ListWorkitemOK(s.T(), context.Background(), nil, s.controller, space.SystemSpace, nil, nil, nil, nil, nil, nil, nil, nil, nil, &offset, nil, nil)
	// then
	if !strings.Contains(*result.Links.First, "page[limit]=20") {
