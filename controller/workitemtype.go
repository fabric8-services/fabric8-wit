...
func (c *WorkitemtypeController) Create(ctx *app.CreateWorkitemtypeContext) error {
	return application.Transactional(c.db, func(appl application.Application) error {
		...
		wit, err := appl.WorkItemTypes().Create(ctx.Context, ctx.Payload.Data.ID, ctx.Payload.Data.Attributes.ExtendedTypeName, ctx.Payload.Data.Attributes.Name, ctx.Payload.Data.Attributes.Description, ctx.Payload.Data.Attributes.Icon, fields, *ctx.Payload.Data.Relationships.Space.Data.ID)
		if err != nil {
			return jsonapi.JSONErrorResponse(ctx, err)
		}
