...
func (s *workItemLinkSuite) SetupTest() {
	...
	fxt := tf.NewTestFixture(s.T(), s.DB,
		...
		tf.WorkItems(4,
			tf.SetWorkItemTitles("bug1", "bug2", "bug3", "feature1"),
...
func (s *workItemLinkSuite) TestCreate() {
	...
	s.T().Run("topology", func(t *testing.T) {
		// Testing simple creation of a link with each topology
		topologies := []link.Topology{
			...
			t.Run(topo.String(), func(t *testing.T) {
				t.Run("ok", func(t *testing.T) {
					...
					fxt := tf.NewTestFixture(t, s.DB,
						tf.CreateWorkItemLinkEnvironment(),
						tf.WorkItems(2, tf.SetWorkItemTitles("bug1", "bug2")),
					...
					compareWithGoldenUUIDAgnostic(t, filepath.Join(s.testDir, "create", "ok.topology."+topo.String()+".golden.json"), workItemLink)
					compareWithGoldenUUIDAgnostic(t, filepath.Join(s.testDir, "delete", "ok.topology."+topo.String()+".headers.golden.json"), res.Header())
				})
			})
		}
	...
	s.T().Run("invalid link type ID", func(t *testing.T) {
		// given
		fxt := tf.NewTestFixture(t, s.DB,
			tf.CreateWorkItemLinkEnvironment(),
			tf.WorkItems(2, tf.SetWorkItemTitles("bug1", "bug2")),
		)
		createPayload := newCreateWorkItemLinkPayload(fxt.WorkItemByTitle("bug1").ID, fxt.WorkItemByTitle("bug2").ID, uuid.Nil)
		// when
		res, jerrs := test.CreateWorkItemLinkBadRequest(t, s.svc.Context, s.svc, s.workItemLinkCtrl, createPayload)
		compareWithGoldenUUIDAgnostic(t, filepath.Join(s.testDir, "create", "invalid_link_type_id.error.golden.json"), jerrs)
		compareWithGoldenUUIDAgnostic(t, filepath.Join(s.testDir, "create", "invalid_link_type_id.headers.golden.json"), res.Header())
	})
}
