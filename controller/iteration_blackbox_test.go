...
func (rest *TestIterationREST) TestCreateChildIteration() {
	...
	rest.T().Run("fail - create same child iteration conflict", func(t *testing.T) {
		...
		compareWithGoldenUUIDAgnostic(t, filepath.Join(rest.testDir, "create", "conflict_for_same_name.errors.golden.json"), jerrs)
	})
	...
	rest.T().Run("fail - create child iteration missing name", func(t *testing.T) {
		...
		compareWithGoldenUUIDAgnostic(t, filepath.Join(rest.testDir, "create", "bad_request_missing_name.errors.golden.json"), jerrs)
	})
	...
	rest.T().Run("fail - create child missing parent", func(t *testing.T) {
		...
		compareWithGoldenUUIDAgnostic(t, filepath.Join(rest.testDir, "create", "bad_request_unknown_parent.errors.golden.json"), jerrs)
	})
	...
	rest.T().Run("unauthorized - create child iteration with unauthorized user", func(t *testing.T) {
		...
		ignoreString := "IGNORE_ME"
		jerrs.Errors[0].ID = &ignoreString
		compareWithGoldenUUIDAgnostic(t, filepath.Join(rest.testDir, "create", "unauthorized.errors.golden.json"), jerrs)
	})
}
