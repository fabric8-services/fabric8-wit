...
type spaceWorkItemTypesSuite struct {
	gormtestsupport.DBTestSuite
	spaceTypeCtrl *SpaceworkitemtypesController
	spaceCtrl     *SpaceController
	svc           *goa.Service
	testDir       string
}

func TestSuiteSpaceWorkItemTypes(t *testing.T) {
	resource.Require(t, resource.Database)
	suite.Run(t, &spaceWorkItemTypesSuite{
		DBTestSuite: gormtestsupport.NewDBTestSuite(""),
	})
}

func (s *spaceWorkItemTypesSuite) SetupSuite() {
	s.DBTestSuite.SetupSuite()
	s.testDir = filepath.Join("test-files", "space_work_item_type")
}

func (s *spaceWorkItemTypesSuite) SetupTest() {
	...
	s.spaceTypeCtrl = NewSpaceworkitemtypesController(s.svc, gormapplication.NewGormDB(s.DB), s.Configuration)
}

func (s *spaceWorkItemTypesSuite) TestList() {
	// given
	fxt := tf.NewTestFixture(s.T(), s.DB, tf.Spaces(1), tf.WorkItemTypes(2))

	s.T().Run("not found using non existing space", func(t *testing.T) {
		// given
		spaceID := uuid.NewV4()
		// when
		res, jerrs := test.ListforspaceSpaceworkitemtypesNotFound(t, nil, nil, s.spaceTypeCtrl, spaceID, nil, nil)
		// then
	...
	s.T().Run("ok", func(t *testing.T) {
		t.Run("generated", func(t *testing.T) {
			// when
			res, witCollection := test.ListforspaceSpaceworkitemtypesOK(t, nil, nil, s.spaceTypeCtrl, fxt.Spaces[0].ID, nil, nil)
			// then
	...
	s.T().Run("ok - using expired IfModifiedSince header", func(t *testing.T) {
		...
		res, witCollection := test.ListforspaceSpaceworkitemtypesOK(t, nil, nil, s.spaceTypeCtrl, fxt.Spaces[0].ID, &lastModified, nil)
		// then
		require.NotNil(t, witCollection)
		require.Nil(t, witCollection.Validate())
	...
	s.T().Run("ok - using IfNoneMatch header", func(t *testing.T) {
		// when
		etag := "foo"
		res, witCollection := test.ListforspaceSpaceworkitemtypesOK(t, nil, nil, s.spaceTypeCtrl, fxt.Spaces[0].ID, nil, &etag)
		// then
		require.NotNil(t, witCollection)
		require.Nil(t, witCollection.Validate())
	...
	s.T().Run("not modified - using IfModifiedSince header", func(t *testing.T) {
		...
		// when
		res := test.ListforspaceSpaceworkitemtypesNotModified(t, nil, nil, s.spaceTypeCtrl, fxt.Spaces[0].ID, &lastModified, nil)
		// then
		safeOverriteHeader(t, res, "Etag", "0icd7ov5CqwDXN6Fx9z18g==")
	...
	s.T().Run("not modified - using IfNoneMatch header", func(t *testing.T) {
		// given
		_, witCollection := test.ListforspaceSpaceworkitemtypesOK(t, nil, nil, s.spaceTypeCtrl, fxt.Spaces[0].ID, nil, nil)
		require.NotNil(t, witCollection)
		// when
		ifNoneMatch := generateWorkItemTypesTag(*witCollection)
		res := test.ListforspaceSpaceworkitemtypesNotModified(t, nil, nil, s.spaceTypeCtrl, fxt.Spaces[0].ID, nil, &ifNoneMatch)
		// then
		safeOverriteHeader(t, res, "Etag", "0icd7ov5CqwDXN6Fx9z18g==")
